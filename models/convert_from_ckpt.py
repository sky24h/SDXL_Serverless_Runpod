...
def convert_ldm_clip_checkpoint(checkpoint):
    checkpoint_path = os.path.join(os.path.dirname(__file__), "../../models/Clip_Model/clip-vit-large-patch14")
    if not os.path.exists(checkpoint_path):
        checkpoint_path = "openai/clip-vit-large-patch32"
    text_model = CLIPTextModel.from_pretrained(checkpoint_path)
    keys = list(checkpoint.keys())
...
def stable_unclip_image_encoder(original_config):
    ...
    if sd_clip_image_embedder_class == "ClipImageEmbedder":
        ...
        raise NotImplementedError(f"Unknown CLIP image embedder class in stable diffusion checkpoint {sd_clip_image_embedder_class}")

    return feature_extractor, image_encoder


def stable_unclip_image_noising_components(original_config, clip_stats_path: Optional[str] = None, device: Optional[str] = None):
    """
...
def convert_controlnet_checkpoint(checkpoint, original_config, checkpoint_path, image_size, upcast_attention, extract_ema):
